...
- **taskId**：\`meme-monitor-<tokenSymbol>\` （小写，短横线分隔）
- **cronExpression**：\`*/5 * * * *\`（每 5 分钟）
- **prompt**：使用下方《监控任务 Prompt 模板》，替换所有 \`<占位符>\`

> 如果 scheduled-tasks MCP 不可用，告知用户需手动回来确认价格，或使用第三方价格提醒工具。

---

### 阶段五：自动监控逻辑（定时任务运行时执行）

**监控任务 Prompt 模板**（创建定时任务时使用，替换占位符后填入 prompt 字段）：
...
- status = "waiting_entry" → 输出"等待买入，跳过" 并退出
- status = "closed" → 输出"仓位已关闭" 并退出
- status = "active" 或 "partial_exit" → 继续

持仓信息：tokenAddress=<TOKEN_ADDRESS>，chain=501，
walletAddress=<WALLET_ADDRESS>，entryPrice=<ENTRY_PRICE>，
...
提取 fromToken.tokenUnitPrice 作为 currentPrice。
输出：当前价 $currentPrice | 入场价 $<ENTRY_PRICE> | 涨跌 X%

### 第三步：出场判断

【翻倍出场】currentPrice >= <ENTRY_PRICE> * 2.0：
卖出 50%（sellAmount = tokenAmount * 0.5）：
onchainos swap swap \\
  --from <TOKEN_ADDRESS> \\
  --to So11111111111111111111111111111111111111112 \\
  --amount <sellAmount> \\
  ...
  --slippage 10
成功后更新 JSON：tokenAmount 减半，status 改为 "partial_exit"，
记录 partialExitPrice 和 partialExitTime。
输出：翻倍出场！已卖 50%，收回本金。txHash: <hash>

【止损清仓】currentPrice <= <ENTRY_PRICE> * 0.5：
onchainos swap swap \\
  --from <TOKEN_ADDRESS> \\
  --to So11111111111111111111111111111111111111112 \\
  --amount <tokenAmount> \\
  ...
  --wallet <WALLET_ADDRESS> \\
  --slippage 15
成功后更新 JSON：status 改为 "closed"，记录 stopLossPrice 和 closedTime。
输出：止损！已全部卖出。txHash: <hash>

【72小时超时】entryTime 至今超过 72 小时：
输出：持仓超72h，请手动决策。当前价 $currentPrice，盈亏 X%
不自动操作。

【正常持有】无触发：
输出：持仓正常，继续持有。$<SYMBOL> $currentPrice（涨跌 X%）
\`\`\`
...
翻倍的 3 笔：
   卖出 50% 收回本金 100 USDT，剩余持仓若再涨 3x → 额外 +150 USDT/笔
   3 笔额外利润 = 450 USDT

止损的 7 笔：
   每笔最多亏 50 USDT，总亏损 = 350 USDT
